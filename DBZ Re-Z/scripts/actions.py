...
def shuffle(group):
    group.shuffle()
    CardsMovedTopRevealCheck2()

def shufflesensei(group):
    group.shuffle()

def RFGAll(group, x = 0, y = 0):
    mute()
    for card in group:
        card.moveTo(card.owner.piles['Removed from game'])
    notify("{} RFG's all cards from {}.".format(me, group.name))

def toggleRevealMode(args):
    mute()
    global revealTopMode
    if revealTopMode:
        revealTopMode = False
        me.piles["Life Deck"][0].isFaceUp = False
        notify("{} hides the top card of their deck.".format(me))
    else:
        revealTopMode = True
        me.piles["Life Deck"][0].isFaceUp = True
        notify("{} sets the top card of their deck to visible.".format(me))

def CardsMovedTopRevealCheck(args):
    if args.player == me:
        if revealTopMode:
            me.piles["Life Deck"][0].isFaceUp = True

def CardsMovedTopRevealCheck2():
        if revealTopMode:
            me.piles["Life Deck"][0].isFaceUp = True
